...
return (
<div
  className={ "z-10 w-screen h-screen absolute bg-black bg-opacity-40 flex justify-center items-center overflow-hidden"
  } onClick={handleClose}>
  <div className={`w-[32rem] bg-white rounded-xl flex flex-col gap-10 p-10 ${style.opened} select-none transition-all`}>
    <div className={"flex"}>
      <RegisterSelectorButton isActive={login} handler={handleLoginSwitch}>
        Login
      </RegisterSelectorButton>

      <RegisterSelectorButton isActive={signUp} handler={handleSignUpSwitch}>
        Register
      </RegisterSelectorButton>
    </div>
